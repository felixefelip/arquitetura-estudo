...
class RbsUsageAnalyzer
  ...
  def detect_forwarding_methods(parse_result, target_class_filter: nil)
    ...
    collector.defs.each do |defn|
      ...
      new_calls = RbsUsageAnalyzer.find_all_nodes(body) { |n| n.is_a?(Prism::CallNode) && n.name == :new && n.receiver && n.arguments }
  ...
  def infer_wrapper_method_param_types(method_name, param_names)
    ...
    @source_files.each do |file|
      ...
      matching_calls = RbsUsageAnalyzer.find_all_nodes(file_result.value) { |n| n.is_a?(Prism::CallNode) && n.name == method_name.to_sym && n.arguments }
  ...
  def collect_local_var_types_for_scope(target_node, parse_result, method_return_types, caller_class_name)
    ...
    all_assignments = RbsUsageAnalyzer.find_all_nodes(enclosing_def) do |n|
    ...
    all_assignments.each do |assign|
      ...
      if assign.value.is_a?(Prism::CallNode)
        if assign.value.receiver.nil?
          ...
          class_name = RbsUsageAnalyzer.extract_constant_path(assign.value.receiver)
          if class_name
  ...
  def resolve_block_param_types(enclosing_def, target_node, local_var_types, method_return_types)
    block_calls = RbsUsageAnalyzer.find_all_nodes(enclosing_def) do |n|
